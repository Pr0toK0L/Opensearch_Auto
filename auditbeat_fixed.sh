...
install_specific_version() {
    ...
    local temp_file="/tmp/auditbeat-$version-amd64.deb"
    ...
    if ! sudo dpkg -i "$temp_file"; then
        echo "Error: Failed to install Auditbeat package"
...
install_latest_version() {
    ...
    # Better way to get latest version - you might need to adjust this based on Elastic's API
...
start_auditbeat() {
    ...
    if ! sudo systemctl start auditbeat; then
        echo "Error: Failed to start auditbeat service"
        exit 1
    fi
...
automated_config() {
    ...
    # Create backup
    if [ -f "$CONFIG_DIR/auditbeat.yml" ]; then
        sudo cp "$CONFIG_DIR/auditbeat.yml" "$CONFIG_DIR/auditbeat.yml.bak"
        if [ $? -eq 0 ]; then
    ...
    sudo tee "$CONFIG_DIR/auditbeat.yml" > /dev/null << 'EOF'
auditbeat.modules:
...
EOF
    ...
    sudo chmod 644 "$CONFIG_DIR/auditbeat.yml"
}
